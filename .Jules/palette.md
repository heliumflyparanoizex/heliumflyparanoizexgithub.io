...
## 2026-04-30 - Explicit Focus Management in Custom Modals
**Learning:** Implementing custom modals or lightboxes without explicit focus management creates significant accessibility barriers, trapping keyboard users or disorienting them when the modal closes. It is critical to track the active element when opening a modal and reliably restore focus to it upon closure, ensuring a predictable navigation flow.
**Action:** Always capture `document.activeElement` into a variable when opening a custom modal, use `setTimeout` to ensure focus correctly shifts to a focusable element within the modal, and explicitly call `.focus()` on the captured element when the modal is closed.
